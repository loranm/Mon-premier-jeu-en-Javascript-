    ...
    ***************************************************************************/
    ...
    var displayGain = function(gain){
        if(gain.length){
            ...
            displayStage(gain[i])// console.log(gain[i].image)
            // var ctx = canvas.getContext('2d');
            // console.log(canvas)
            // ctx.drawImage(mySpriteSheet,gain[i].sx,gain[i].sy,gain[i].sw,gain[i].sh,gain[i].dx,gain[i].dy,gain[i].dw,gain.dh)
            // // elements[elements.length] = gain[i];
        }
    }
    /***************************************************************************
            ...
            for (var i = 0; i < stage.length; i++) {
                if (stage[i].bonus) {
                    if (this.dx  < stage[i].dx + stage[i].dw &&
                        ...
                        this.dh + this.dy - 120> stage[i].dy) {
                        gain[gain.length] = stage[i];
                        stage.splice(i, 1);
                    };
                };
            };

...
var Heros = function(options) {
  ...
  this.testCollision = function(){
    for (var i = 0; i < stage.length ; i ++){
      if(stage[i].platform){
        console.log(this.dx+5)
        if(this.dx+5 == stage[i].dx){
            this.canGoRight = false
        };
      };
    };
  ...
  //     };
      ...
      this.vsPlatform = function(currentPlatform) {
        //la tête du héros est plus haut que le bas de la plafform

        if (this.dy < currentPlatform.dy+currentPlatform.dh){
          console.log('le héros est bloqué')
          this.dx = currentPlatform.dx;
        };

        //la tête du héros est plus basse que le bas de la platform
        if (this.dy > currentPlatform.dy + currentPlatform.dh){console.log('on s\'en fout')};

        //les pieds du héros sont au-dessus du bas la plateforme
        if (this.dy+this.dh < currentPlatform.dy+currentPlatform.dh){
          console.log('on monte')
          if (this.dx + this.dw > currentPlatform.dx && this.dx + this.dw < currentPlatform.dx + currentPlatform.dw) {
